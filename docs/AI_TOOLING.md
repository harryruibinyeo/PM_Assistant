...
## MCP: one tool implementation, two agents

The **[Model Context Protocol](https://modelcontextprotocol.io)** is what lets both Hermes profiles call into the same Python tool codebase (`pm-chaser-mcp`) over a standard interface instead of each having its own bespoke integration. Both profiles run against the same `pmchaser/` implementation and the same database — as of the per-profile tool exposure work, each connects to its own server *instance* (a separate process, advertising only the tool subset that profile actually uses) rather than one shared instance, but it's still exactly the same underlying code either way. It's the reason a rule change in one place (e.g., how the re-ping floor is computed) automatically applies identically to the manager's live chat and the automated chase sweep — both are MCP clients hitting the exact same tool implementations. Full tool catalog: [`MCP_TOOLS.md`](./MCP_TOOLS.md).
...
- **Splitting one shared MCP server into two profile-scoped instances.** Even after disabling the discovery bridge, `pmchaser-bot`'s 15-minute cron job was still paying for all 16 (then) tools' schemas every run despite its own `SKILL.md` calling only 5 of them by name — a ~3x token overpay, measured. `pmchaser/mcp/profiles.py` now derives each profile's real tool list from what its own instructions actually reference, and each profile connects to its own server instance advertising only that subset. See [`MCP_TOOLS.md`](./MCP_TOOLS.md#which-tools-each-profile-actually-gets).
- **Proof-of-send instead of narrated confidence.** A local model, asked to send a message, would sometimes write a confident closing line — "Pinged Daniel about the deck" — without ever calling `telegram_send_message`. Fixed structurally: a scheduled job now hard-fails if it ends without a real send tool call ever succeeding, rather than trusting the model's own summary of what it did. **A more targeted recurrence of the same failure class** was later found live: asked to chase Henry, the model called `chase_now` correctly, got real data back, then narrated a send it never made — the job-level hard-fail caught it after the fact, but didn't stop it from happening. Fixed two ways at once: `chase_now`/`get_chase_plan` now embed an `action_required` field directly in their response data, restating in plain language exactly which tool call still needs to happen before anything can be reported as sent; and a new composite tool, `record_reply_outcome`, collapsed a separate three-call sequence (update the task, acknowledge the reply, notify the manager) that had the identical "complete the first call, narrate the rest" shape into one call, removing the mid-sequence abandonment point rather than just detecting it afterward.
