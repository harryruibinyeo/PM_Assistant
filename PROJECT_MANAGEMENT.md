...
**A second, smaller literalism bug surfaced right after:** the recap-trim fix from earlier (step 9) had used a fictional example — `"e.g., 'Sent to Henry and Jeffyeo, escalated 2 to Jeffrey.'"` — and the model echoed the *names* from the example into a run where nobody was actually chased, producing a factually wrong sign-off. Same root cause as the escalation bug: this model pattern-matches provided examples more literally than the 27B model did. Fixed in two passes — first to stop copying names (still left an awkward but no-longer-false "Sent to Jeffrey, escalated 3 to Jeffrey"), then to stop forcing both clauses into one sentence when only one applies. Confirmed clean on the third try: `"Escalated 3 tasks from Henry to Jeffrey."`
...
Odysseus's only remaining role was Jeffrey creating/editing tasks by chatting with it. Built a standalone replacement: a **second, dedicated Telegram bot** (`@taskmanager_threeporkchops_bot`, separate token) that Jeffrey talks to directly, with real multi-turn conversation memory — unlike `run.py`'s chase/digest jobs, which fire once and exit, every message here is a genuine back-and-forth ("add a task for Henry" → "what's the deadline?" → "Friday 5pm").

**Key design decisions:**
- **No `/start`-with-code linking flow needed.** In Telegram, a private chat's `chat.id` is the user's own numeric Telegram ID, not bot-scoped — confirmed for real before writing any code (messaged the new bot from Jeffrey's account, checked `getUpdates`, got back the same `chat_id` — `8713903593` — already on file from the employee bot). Supplied via `MANAGER_CHAT_ID` in `agent/.env` rather than resolved through `list_people()`, which deliberately never exposes the raw `telegram_chat_id` (only `is_linked`/`link_code`/`link_url`) — a real gap between the original plan and what the tool layer actually returns, caught before writing the lookup code.
